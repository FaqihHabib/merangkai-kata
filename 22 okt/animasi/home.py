...
from kivy.animation import Animation  # Tambahkan animasi
...
class Home(Screen):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            self.bg = Rectangle(source='./gambar/background.png', size=Window.size, pos=self.pos)
        ...
        self.judul_gambar = Image(source='gambar/logo2.jpg', size_hint=(None, None), size=(800, 400), pos_hint={'center_x': 0.5, 'center_y': 0.7})
        layout.add_widget(self.judul_gambar)

        layout.add_widget(Label(size_hint_y=None, height=50))

        # Tombol "Mulai" menggunakan SoundImageButton
        ...
        # Jalankan animasi untuk logo dan tombol "Mulai"
        self.animate_logo()
    ...
    def toggle_music(self, instance):
        if self.music_on:
            self.music.stop()
            instance.source = 'gambar/music_off.jpg'
        else:
            self.music.play()
            instance.source = 'gambar/music_on.jpg'
