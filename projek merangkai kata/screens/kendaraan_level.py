...
class KendaraanLevel(Screen):
    def __init__(self, **kwargs):
        ...
        self.levels = KENDARAAN_QUESTIONS
        ...
        with self.canvas.before:
            self.bg = Rectangle(source='gambar/bgkendaraan.png', size=Window.size, pos=self.pos)
        ...
        self.kendaraan_image = Image(
            source="",
            size_hint=(None, None),
            size=(200, 200),
            pos_hint={'center_x': 0.5, 'center_y': 0.6}
        )
        layout.add_widget(self.kendaraan_image)
    ...
    def load_level(self, index):
        ...
        self.kendaraan_image.source = level['kendaraan_image']
        ...
        for row in range(rows):
            ...
            for i, letter in enumerate(row_letters):
                btn = SoundImageButton(
                    source=f'gambar/huruf_kendaraan/{letter.lower()}.png',
                    size_hint=(None, None),
                    size=(100, 100),
    ...
    def go_back_to_level_select(self, instance):
        ...
        print(f'ini adalah {self.current_level_index} dan {self.pick_screen}')
