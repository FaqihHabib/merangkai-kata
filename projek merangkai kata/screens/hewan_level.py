...
class HewanLevel(Screen):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            self.bg = Rectangle(source='gambar/bghewan.png', size=Window.size, pos=self.pos)
        ...
        self.hewan_image = Image(
            source="",
            size_hint=(None, None),
            size=(200, 200),
            pos_hint={'center_x': 0.5, 'center_y': 0.6}
        )
        layout.add_widget(self.hewan_image)
        ...
        for i in range(3):
            heart = Image(
                ...
                pos_hint={'right': 1 - (i * spacing/200), 'top': 1} # Posisi absolut
            )
    ...
    def set_pick_screen(self, pick_screen):
        ...
        print(f"ini adalah pick screen {pick_screen}")
    ...
    def load_level(self, index):
        ...
        # Update gambar hewan
        self.hewan_image.source = level['hewan_image']
        ...
        for row in range(rows):
            ...
            for i, letter in enumerate(row_letters):
                btn = SoundImageButton(
                    source=f'gambar/huruf_hewan/{letter.lower()}.png',
                    size_hint=(None, None),
                    size=(100, 100),
    ...
    # Cek apakah ini level terakhir
        if self.current_level_index == 9 and self.pick_screen:
            print("Mencoba membuka level kendaraan...")  
            PILIH_LEVEL['kendaraan'] = 'unlocked'
            self.pick_screen.level_status = PILIH_LEVEL
            print(f"Status level setelah update: {self.pick_screen.level_status}")  
    ...
    def go_back_to_level_select(self, instance):
        ...
        if self.current_level_index == 9 and self.pick_screen:
            PILIH_LEVEL['kendaraan'] = 'unlocked'
            self.pick_screen.level_status = PILIH_LEVEL
            print("Level kendaraan dibuka sebelum kembali ke menu")
